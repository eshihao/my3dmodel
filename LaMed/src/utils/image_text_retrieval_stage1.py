...
model_path = "/path/to/stage1_CLIP_model"
model_name = "Stage_1"
...
def compute_all_sample(test_set):
    ...
    for image in tqdm(image_input):
        with torch.inference_mode():
            image_features.append(model.encode_image(image.to(device=device))[:, 0].to(device="cpu"))
    ...
    for return_n in [1, 5, 10, 50]:
        for i in tqdm(range(image_features.shape[0])):
            ...
            ratios_internal = []
            # Calculate the similarity between the labels of these images and the label of the current image.
