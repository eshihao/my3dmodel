...
BASE_MODEL_PATH = "/data/esh/HSENet/phi-4-mini-instruct/Phi-4-mini-instruct"
STAGE2_VISION_PATH = "/data/esh/HSENet/Preprint/LaMed/output_new/stage2/model.safetensors"
STAGE3_WEIGHTS_PATH = "/data/esh/HSENet/Preprint/LaMed/output_new/stage3_2/mm_projector_and_lora.bin"
PROJ_OUT_NUM = 144
...
def main():
    ...
    # ================= [必须同步 2] 双路 Projector 加载 =================
    if os.path.exists(STAGE3_WEIGHTS_PATH):
        finetuned_weights = torch.load(STAGE3_WEIGHTS_PATH, map_location="cpu")
        
        projector_weights = {}
        for k, v in finetuned_weights.items():
            if 'mm_projector2' in k: projector_weights[k[k.find('mm_projector2'):]] = v
            elif 'mm_projector' in k: projector_weights[k[k.find('mm_projector'):]] = v
        if len(projector_weights) > 0: model.get_model().load_state_dict(projector_weights, strict=False)

        embed_weights = {k: v for k, v in finetuned_weights.items() if 'embed_tokens' in k or 'lm_head' in k}
        if len(embed_weights) > 0: model.load_state_dict(embed_weights, strict=False)
        
        lora_weights = {k.replace("base_model.model.", ""): v for k, v in finetuned_weights.items() if 'lora' in k}
        if len(lora_weights) > 0: set_peft_model_state_dict(model, lora_weights)
